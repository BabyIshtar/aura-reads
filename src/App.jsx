...
async function fetchSongMedia(song, artist) {
  // Spotify Web API can be blocked by account restrictions.
  // Use Apple/iTunes first for reliable public previews + artwork, then Deezer as backup.
  ...
  for (const source of previewSources) {
    ...
    if (previewMedia?.previewUrl || previewMedia?.albumArt) {
      return {
        ...previewMedia,
...
const SPOTIFY_DISCOVERY_QUERY_LIMIT = 16;
const SPOTIFY_DISCOVERY_PICK_POOL = 90;
...
function spotifyTrackGenreLooksAllowed(track = {}, searchGenreKey, genreSettings = DEFAULT_GENRE_SETTINGS) {
  ...
  const matchedKeys = detectGenreKeysFromText(artistGenres.join(" "));
...
const AURA_DISCOVERY = {
  ...
  warmDreamscape: {
    ...
    reasons: [
      ...
      "The colors feel warm and intimate, so Aura pulled a fresh soft track from live music search."
    ]
  },
  editorialLuxury: {
    queries: [
...
async function fetchSpotifyDiscovery(auraKey, excludedKeys = new Set(), genreSettings = DEFAULT_GENRE_SETTINGS, imageBrain = null) {
  const queries = discoveryQueriesForAura(auraKey, genreSettings, imageBrain).slice(0, SPOTIFY_DISCOVERY_QUERY_LIMIT);
  const candidates = [];
  const seen = new Set();
  ...
  for (const queryItem of queries) {
    ...
    try {
      ...
      for (const track of tracks) {
        ...
        if (isExcludedTrack(track.name, artistName, excludedKeys)) continue;

        const mainstreamMetadata = {
          spotifyTrackId: track.id,
          spotifyUrl: track.external_urls?.spotify || "",
        ...
        if (!isUsMainstreamEligible(mainstreamMetadata, true)) continue;
        if (!spotifyTrackGenreLooksAllowed(track, searchGenreKey, genreSettings)) continue;
  ...
  const pickedBundle = randomItem(ranked.slice(0, Math.min(ranked.length, 35))) || ranked[0];
...
function spotifyCandidateQueries(auraKey, genreSettings = DEFAULT_GENRE_SETTINGS, imageBrain = null) {
  const enabled = enabledGenreKeys(genreSettings);
  const auraTerms = {
    grungeNoir: ["dark alternative", "moody rock", "darkwave", "shoegaze", "post punk"],
    neonNightlife: ["night drive", "dark pop", "melodic rap", "electronic pop", "club rap"],
    warmDreamscape: ["r&b", "neo soul", "dream pop", "indie soul", "warm pop"],
...
async function buildFreshAuraResult(auraKey, colors = ["6d5dfc", "19d8ff", "ff3df2"], genreSettings = DEFAULT_GENRE_SETTINGS, imageBrain = null) {
  ...
  if (!liveTrack?.song || !liveTrack?.artist) {
    try {
      liveTrack = await fetchDeezerDiscovery(auraKey, excludedKeys, genreSettings, imageBrain);
    } catch (error) {
      console.warn("Deezer aura discovery failed", error);
    }
  }
...
const entry = {
  ...
  async function togglePreview() {
    ...
    try {
      ...
      if (!previewUrl) {
        setPreviewError("No preview is available for this track. Try Similar Track.");
        return;
      }
    ...
    <main style={gradientStyle} className={`relative min-h-screen overflow-hidden bg-[#07080a] text-white ${isCompactDevice || isStandalonePwa() ? "aura-mobile-stable" : ""}`}>
      ...
      <audio
        ...
        onError={() => {
          ...
          setAudioReactive(false);
          setPreviewLoading(false);
          setPreviewError("This preview link failed to load. Tap Similar Track for another match.");
        }}
      />
            ...
            <div className="relative z-10 flex min-h-dvh flex-col items-center justify-between px-6 py-10">
              ...
              <div className="flex flex-col items-center">
                <div className="relative mb-8">
                  ...
                  <img
                    src={result.albumArt}
                    onError={(event) => { event.currentTarget.src = generatedAlbumArt(result.song, result.artist, result.colors || colors); }}
                    alt={result.song}
              ...
              <motion.div key="result" initial={{ opacity: 0, y: 34, scale: 0.97, filter: "blur(14px)" }} animate={{ opacity: 1, y: 0, scale: 1, filter: "blur(0px)" }} exit={{ opacity: 0, y: -18, scale: 0.98, filter: "blur(10px)" }} transition={{ duration: 0.82, ease: CASCADE_EASE }} className="w-full">
                ...
                <div className="aura-result-card rounded-[2rem] p-5 text-left shadow-xl shadow-black/40">
                  ...
                  <div className="aura-song-pill flex items-center gap-3 rounded-[1.5rem] p-3">
                    <img src={result.albumArt} onError={(event) => { event.currentTarget.src = generatedAlbumArt(result.song, result.artist, result.colors || colors); }} alt={`${result.song} album art`} className="h-16 w-16 rounded-2xl object-cover shadow-xl shadow-black/40" />
                  ...
                  <button onClick={togglePreview} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(90deg,var(--aura-a),var(--aura-b),var(--aura-c))] px-4 py-3.5 text-sm font-black text-black shadow-[0_0_42px_color-mix(in_srgb,var(--aura-b)_45%,transparent)] transition duration-500 ease-out hover:scale-[1.015] active:scale-[0.985]">
                    ...
                    {previewLoading ? "Finding preview..." : playing ? "Pause preview" : "Play preview"}
                  </button>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {auraHistory.slice(0, 6).map((item) => (
                <div key={item.id} className="aura-history-card min-w-[138px] rounded-[1.4rem] p-2">
                  <img src={item.albumArt} onError={(event) => { event.currentTarget.style.display = "none"; }} alt="" className="h-20 w-full rounded-[1rem] object-cover" />
